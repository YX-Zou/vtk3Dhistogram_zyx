...
# Changed By Yuxiong Zou   2019.12.1
...
class VTK3Dhist():
    ...
    def writeVTK(self,filename,xx,yy,zz):
        ...
        #output polygons
        print(f_out,"POLYGONS ", n,n*5, file = f_out)  #polygon_num polygon_num*5
        for i in range(n):
    ...
    def write3DhistogramVTK(self,filename,coeff,coeff_magnitude):
        print("writing Polydata into a VTK file")
        f_out = open(filename,'w')
        ...
        m,n = self.xx.shape
        print("POINTS   ", n*4 + 1," float", file = f_out)  ###index is from 0 in VTK; here we include the origin
        #output points
        print(0,0,0, file = f_out) #output the origin
        for i in range(n):#n elements or polygons
            for j in range(m):
                print(self.xx[j,i]*coeff[i],self.yy[j,i]*coeff[i],self.zz[j,i]*coeff[i], file = f_out)
        #output polygons
        print("POLYGONS ", n*5,n*21, file = f_out)  #polygon_num polygon_num*5
        for i in range(n):
            #print>>f_out,"4 ",i*4,i*4+1,i*4+2,i*4+3
            print(self.drawBar(i*4+1,i*4+2,i*4+3,i*4+4,0), file = f_out)
        #output magnitude
        print("CELL_DATA ", n*5, file = f_out)  #
        print("SCALARS density_scalars float 1", file = f_out)
        print("LOOKUP_TABLE default", file = f_out)
        #print values of all cells (polygons)
        for i in range(n):
            print(coeff[i],coeff[i],coeff[i],coeff[i],coeff[i], file = f_out)
        print("SCALARS magnitude_scalars float 1", file = f_out)
        print("LOOKUP_TABLE default", file = f_out)
        for i in range(n):
            print(coeff_magnitude[i],coeff_magnitude[i],coeff_magnitude[i],coeff_magnitude[i],coeff_magnitude[i], file = f_out)
#        print("CELL_DATA ", n*5, file = f_out)  #
#        print("SCALARS magnitude_scalars float 1", file = f_out)
#        print("LOOKUP_TABLE default", file = f_out)
#        for i in range(n):
#            print(coeff_magnitude[i],coeff_magnitude[i],coeff_magnitude[i],coeff_magnitude[i],coeff_magnitude[i], file = f_out)
    ...
    def VTK3Dhistogram_basic(self,file_input,file_output):#three-column data
        ...
        coeff_magnitude = np.zeros(self.elements_num)
        for i in range(NumData):
            j1,j2 = self.find_j(cube_x[i],cube_y[i],cube_z[i])
            coeff[j1-1] += 1
            #coeff[j2-1] += 1
            coeff_magnitude[j1-1] += magnitude[i]

        coeff_magnitude = coeff_magnitude/coeff
        coeff= (coeff/NumData)/(4.0*np.pi/self.elements_num)

        self.write3DhistogramVTK(file_output+".vtk",coeff,coeff_magnitude)
